...
SELECT
   concat(salary, last_name) AS result -- OK: By promotion
FROM
   employees;
...
SELECT
   '900303-1234567',
   -- 주어진 문자열의 가장 마지막 문자의 인덱스 번호는 -1부터 시작하고
   -- 왼쪽으로 음수인덱스 번호가 작아진다!!!
...
-- ------------------------------------------------------
SELECT
   lpad('MILLER', 10, '*') AS val1,
   lpad('MILLER', 10) AS val2
FROM
   dual;
...
SELECT
   ...
   rpad('MILLER', 3) AS R3,     -- OK
   lpad('MILLER', 3) AS R4,     -- OK
   lpad('123456-1234567', 6) AS R5     -- OK
FROM
   dual;
...
SELECT
   ...
   -- rpad/lpad 함수의 두번째 매개변수에 작은 숫자를 지정해서,
   -- 문자열을 빠르게/쉽게 잘라낼수 있다!!!
