...
SELECT
    ...
    departments.department_name             -- 부모(부서)의 컬럼
FROM    -- (1) CP 연산발생
    employees,
    departments
...
SELECT
    last_name,
    department_name,
    employees.department_id    -- ORA-00918: column ambiguously defined
FROM
    employees,
    departments
WHERE
...
-- 아래처럼 조건을 나열하는 것은, 현업에서는 바람직하지 않습니다.
-- 조인조건 > 체크조건
WHERE
...
--     -- e.salary BETWEEN 1000 AND 2999;

WHERE
    last_name = 'Whalen';
    AND g.grade_level IN (
            CASE
